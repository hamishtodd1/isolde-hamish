        ...
        if m is not None and not m.deleted:
            try:
                ...
                with busy_cursor(self.session, 'Adding hydrogens...'):
                    ...
                    # metal (it assumes coordination). Amides are never deprotonated,
                    # so restore those H -- otherwise a metal-site residue is one atom
                    # short of its template and its unit row never clears.
                    repair_backbone_amide_hydrogens(self.session, m)
